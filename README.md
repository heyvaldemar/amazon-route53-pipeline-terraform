...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14908/badge)](https://www.bestpractices.dev/projects/14908)
...
Every release from v1.1.4 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `amazon-route53-pipeline-terraform-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `amazon-route53-pipeline-terraform-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `amazon-route53-pipeline-terraform-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob amazon-route53-pipeline-terraform-<tag>.tar.gz \
  --bundle amazon-route53-pipeline-terraform-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/amazon-route53-pipeline-terraform/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact amazon-route53-pipeline-terraform-<tag>.tar.gz \
  --provenance-path amazon-route53-pipeline-terraform-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/amazon-route53-pipeline-terraform
```
